...
export async function POST(request: Request) {
  ...
  try {
    ...
    await env.ASSETS.put(crestKey, await candidate.arrayBuffer(), { httpMetadata: { contentType: candidate.type || "image/webp" } });
    ...
    if (existing?.crest_key && existing.crest_key !== crestKey) await env.ASSETS.delete(existing.crest_key).catch(() => {});
