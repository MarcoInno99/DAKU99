...
export function CrestUploader({ team, compact = false }: { team: Team; compact?: boolean }) {
  ...
  return <section className={compact?"crest-page compact":"crest-page"}><div className="crest-copy"><p className="eyebrow">La mia società</p><h1>{team.name}</h1><p>Gestisci direttamente qui l’identità pubblica del club, il nome dell’allenatore e lo stemma.</p><div className="coach-field"><label>Nome dell’allenatore<input value={coachName} maxLength={60} placeholder="Es. Marco Izzo" onChange={event => setCoachName(event.target.value)}/></label><button className="secondary-button" disabled={busy || coachName.trim().length < 2} onClick={saveCoach}>Salva allenatore</button></div><div className="crest-rules"><span>PNG, JPG o WEBP</span><span>File originale fino a 20 MB</span><span>Formato consigliato 1:1</span></div></div><div className="crest-card"><div className="crest-preview">{url ? <img src={url} alt={`Stemma ${team.name}`}/> : <div>{team.name.split(" ").map(x => x[0]).join("").slice(0,3)}</div>}</div>{compact && <div className="company-quick-actions"><a className="primary-link" href="/formazione">Schiera la formazione</a><a className="secondary-link" href={`/club/${team.slug}`}>Apri rosa completa</a></div>}<h2>{team.name}</h2><p>{coachName ? `Allenatore: ${coachName}` : exists ? "Stemma attualmente pubblicato" : "Nessuno stemma caricato"}</p><label className="crest-file"><input type="file" accept="image/png,image/jpeg,image/webp" disabled={busy} onChange={event => choose(event.target.files?.[0] ?? null)}/><span>{busy ? "Preparazione…" : file ? file.name : "Scegli immagine"}</span></label><button onClick={save} disabled={busy || !file}>{busy ? "Attendi…" : "Salva stemma squadra"}</button>{message && <small>{message}</small>}</div></section>;
}
